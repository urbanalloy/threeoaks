...
LPDIRECT3DVERTEXBUFFER9 g_pPyramidVertexBuffer = NULL;
...
void initBuffer(LPDIRECT3DVERTEXBUFFER9* buffer, FracVertex * vertices, int numVertices)
{
	g_pd3dDevice->CreateVertexBuffer( numVertices*sizeof(FracVertex),    
		...
		NULL );
	void *pVertices = NULL;
	(*buffer)->Lock( 0, numVertices*sizeof(FracVertex), (void**)&pVertices, 0 );
...
void Fractal::RenderBase(ColorRGB color)
{
	...
	{
		g_pd3dDevice->SetFVF( FVF_FRACVERTEX );
		g_pd3dDevice->SetStreamSource( 0, g_pPyramidVertexBuffer, 0, sizeof(FracVertex) );
		g_pd3dDevice->DrawPrimitive( D3DPT_TRIANGLESTRIP, 0, 4 );
	}
	else
	{
		g_pd3dDevice->SetFVF( FVF_FRACVERTEX );
		g_pd3dDevice->SetStreamSource( 0, g_pCubeVertexBuffer, 0, sizeof(FracVertex) );
