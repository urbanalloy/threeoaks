...
LPDIRECT3DVERTEXBUFFER9 g_pSphereVertexBuffer = NULL; // Vertices Buffer
LPDIRECT3DINDEXBUFFER9 g_pSphereIndexBuffer = NULL; // Index Buffer
int quality;
...
void C_Sphere::Update() {
    ...
    g_pd3dDevice->CreateIndexBuffer(numIndices * sizeof(short), D3DUSAGE_DYNAMIC,
		D3DFMT_INDEX16, D3DPOOL_DEFAULT, &g_pSphereIndexBuffer, NULL);
	
	short * p_DeviceIBMem;
	g_pSphereIndexBuffer->Lock(0, numIndices * sizeof(short), (void**)&p_DeviceIBMem, 0);
	...
	g_pSphereVertexBuffer->Lock(0, numVertices*  sizeof(SPHEREVERTEX), (void**)&vertexMemory, 0);
...
void C_Sphere::Render3D() {
	...
	// Setup D3D stuffs
	g_pd3dDevice->SetStreamSource(0, g_pSphereVertexBuffer, 0, sizeof(SPHEREVERTEX));
	g_pd3dDevice->SetFVF(D3DFVF_SPHEREVERTEX);
	g_pd3dDevice->SetIndices(g_pSphereIndexBuffer);

	//g_pd3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, numVertices, 0, numTriangles);
	g_pd3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, numVertices, 0, numTriangles, 1);
