...
void FractalController::IncrementSelectedXYZ(float dx, float dy, float dz)
{
	...
	{
		case 0:
			myDesiredFractalData.transforms[mySelectedTransform].translation.x += 0.05f * dx;
			myDesiredFractalData.transforms[mySelectedTransform].translation.y += 0.05f * dy;
			myDesiredFractalData.transforms[mySelectedTransform].translation.z += 0.05f * dz;
			break;
		case 1:
			myDesiredFractalData.transforms[mySelectedTransform].rotation.x += 1.5f * dx;
			myDesiredFractalData.transforms[mySelectedTransform].rotation.y += 1.5f * dy;
			myDesiredFractalData.transforms[mySelectedTransform].rotation.z += 1.5f * dz;
			break;
		default:
			myDesiredFractalData.transforms[mySelectedTransform].scaling.x *= (1.0f + 0.05f * dx);
			myDesiredFractalData.transforms[mySelectedTransform].scaling.y *= (1.0f + 0.05f * dy);
			myDesiredFractalData.transforms[mySelectedTransform].scaling.z *= (1.0f + 0.05f * dz);
			break;
	}
}
...
*/
			myDesiredFractalData.transforms[ind].rotation.x = myAnimationBase.transforms[ind].rotation.x + myCounter*(0.1f + 0.2f*i/numTrans - 0.3f*((3*ind)%7)-2.0f);
			myDesiredFractalData.transforms[ind].rotation.y = myAnimationBase.transforms[ind].rotation.y + (float)(2.0f*i-numTrans) / (float)numTrans* myCounter*(0.2f + 0.4f*i/numTrans);
			myDesiredFractalData.transforms[ind].rotation.z = myAnimationBase.transforms[ind].rotation.z + myCounter*(0.16f + 0.4f*i/numTrans - 0.4f*((5*ind)%9)-4.0f);
      ...
      myDesiredFractalData.transforms[ind].rotation.z -= (int)(myDesiredFractalData.transforms[ind].rotation.z/(2.0f*M_PI)) * 2.0f*M_PI;
			...
			myDesiredFractalData.transforms[ind].translation.x
			= myAnimationBase.transforms[ind].translation.x *0.5f + 0.7f*(2*(ind%2)-1) * sin((0 + 0.5f*r)*myCounter/10.0f);
			myDesiredFractalData.transforms[ind].translation.y
			= myAnimationBase.transforms[ind].translation.y *0.6f + 0.7f*(2*(ind%2)-1) * sin((0 - 1.4f*r)*myCounter/10.0f);
			myDesiredFractalData.transforms[ind].translation.z
			= myAnimationBase.transforms[ind].translation.z *0.9f - 0.2f*(2*(ind%2)-1) * sin((0 - 2.4f*r)*myCounter/10.0f);

			myDesiredFractalData.transforms[ind].scaling.x
			= myAnimationBase.transforms[ind].scaling.x * (1.0f + 0.3f*(2*(ind%2)-1)* sin((r - 0.49f*(0.2f + r)*myCounter)));
			myDesiredFractalData.transforms[ind].scaling.y
			= myAnimationBase.transforms[ind].scaling.y * (1.0f + 0.3f*(2*(ind%2)-1)* sin((r - 0.5f*(0.2f + r)*myCounter)));
			myDesiredFractalData.transforms[ind].scaling.z
			= myAnimationBase.transforms[ind].scaling.z * (1.0f + 0.3f*(2*(ind%2)-1)* sin((r - 0.51f*(0.2f + r)*myCounter)));
...
void MakeCantor(FractalData * data)
{
	...
	data->transforms[0].scaling.y = 1.0f/3.0f;
	data->transforms[1].scaling.y = 1.0f/3.0f;
	
	data->transforms[0].translation.y = 1.0f; 
	data->transforms[1].translation.y = -1.0f; 
...
void MakeCube(FractalData * data, int type)
{
	...
	{
		data->transforms[i].scaling.x = data->transforms[i].scaling.y = data->transforms[i].scaling.z = 1.0f/3.0f;
