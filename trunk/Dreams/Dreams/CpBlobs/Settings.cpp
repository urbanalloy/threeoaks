...
void SetDefaults()
{
	...
	strcpy( g_strCubemap, "Resources\\data\\cube.dds" );
	strcpy( g_strDiffuseCubemap, "Resources\\data\\cube_diffuse.dds" );
	strcpy( g_strSpecularCubemap, "Resources\\data\\cube_specular.dds" );
	...
	if (doc.Load(szXMLFile) >= 0)
	{
		...
		while(node > 0)
		{
			...
			if (childNode = doc.GetChildNode(node,"showcube"))
				g_bShowCube = !_strcmpi(doc.GetNodeText(childNode),"true");
			...
			if ( childNode = doc.GetChildNode( node, "cubemap" ) )
				sprintf((char*)&g_strCubemap, "%s\\%s", contentPath, doc.GetNodeText( childNode ));				
			if ( childNode = doc.GetChildNode( node, "diffusecubemap" ) )
				sprintf((char*)&g_strDiffuseCubemap, "%s\\%s", contentPath, doc.GetNodeText( childNode ));				
			if ( childNode = doc.GetChildNode( node, "specularcubemap" ) )
				sprintf((char*)&g_strSpecularCubemap, "%s\\%s", contentPath, doc.GetNodeText( childNode ));	
