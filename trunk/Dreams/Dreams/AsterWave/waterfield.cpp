...
WaterField::WaterField(LPDIRECT3DDEVICE9 device)
{
	Init(device, -10,10,-10,10,80,80,0, 0.5f, 0.05f, 1.0f, 0.04f, false);
}

WaterField::WaterField(LPDIRECT3DDEVICE9 device, float xmin, float xmax, float ymin, float ymax, int xdivs, int ydivs, float height, float elasticity, float viscosity, float tension, float blendability, bool textureMode)
{
	Init(device, xmin, xmax, ymin, ymax, xdivs, ydivs, height, elasticity, viscosity, tension, blendability, textureMode);
}
...
************************************************************/
void WaterField::Init(LPDIRECT3DDEVICE9 device, float xmin, float xmax, float ymin, float ymax, int xdivs, int ydivs, float height, float elasticity, float viscosity, float tension, float blendability, bool textureMode)
{
	myXmin = xmin;
	myYmin = ymin;
	myXmax = xmax;
	...
	if (m_textureMode)
	{
		device->CreateVertexBuffer(2 * myYdivs*  sizeof(TEXTUREDVERTEX),
			D3DUSAGE_WRITEONLY, 
	...
	{
		device->CreateVertexBuffer(2 * myYdivs*  sizeof(COLORVERTEX),
			D3DUSAGE_WRITEONLY, 
			D3DFVF_COLORVERTEX,
			D3DPOOL_DEFAULT, 
			&g_pVertexBuffer,
...
************************************************************/
void WaterField::Render(LPDIRECT3DDEVICE9 device)
{
	int i, j, k;
	...
	if (!m_textureMode){
		...
		for(i=0; i<myXdivs-1; i++)
		{
			...
			// Draw it
			device->SetFVF(D3DFVF_COLORVERTEX);
			device->SetStreamSource(0, g_pVertexBuffer, 0, sizeof(COLORVERTEX));
			device->DrawPrimitive(D3DPT_TRIANGLESTRIP, 0, 2*(myYdivs-1));
	...
	else
	{
		...
		for(i=0; i<myXdivs-1; i++)
		{
			...
			device->SetStreamSource(0, g_pVertexBuffer, 0, sizeof(TEXTUREDVERTEX));
			device->DrawPrimitive(D3DPT_TRIANGLESTRIP, 0, 2*(myYdivs-1));
		}
	}
}
