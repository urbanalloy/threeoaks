...
#define NUM_MODES 7
	...
	g_pd3dDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 1, v, sizeof(TEXVERTEX));	
...
{ 
	...
	sprintf((char*)&szTexPath, "%s\\%s", contentPath, "Resources\\data\\");	
...
BOOL doInit()
{
	...
	if(FAILED(g_pd3dDevice->CreateTexture(FXW2, FXH2, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &lpD3DVS[0], NULL)))
		return FALSE;

	if(FAILED(g_pd3dDevice->CreateTexture(FXW2, FXH2, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &lpD3DVS[1], NULL)))
		return FALSE;
