...
{
	LPDIRECT3DDEVICE9	d3dDevice	= render->GetDevice();

	d3dDevice->CreateVertexBuffer( 24*sizeof(TBGRenderVertex), D3DUSAGE_WRITEONLY, TBGRenderVertex::FVF_Flags, D3DPOOL_DEFAULT,	&m_VertexBuffer, NULL);
	void *pVertices	= NULL;
	m_VertexBuffer->Lock( 0, sizeof(gBGVertices), (void**)&pVertices, 0 );
...
bool		CBackground::Draw(CRenderD3D* render)
{
	...
	d3dDevice->SetStreamSource( 0, m_VertexBuffer, 0, sizeof(TBGRenderVertex) );
	d3dDevice->SetFVF( TBGRenderVertex::FVF_Flags );
...
bool	CBackground::CreateTexture(CRenderD3D* render)
{
	...
	D3DLOCKED_RECT	rect;
	DVERIFY( d3dDevice->CreateTexture(width, height, 1,	0, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &m_Texture, NULL) );
