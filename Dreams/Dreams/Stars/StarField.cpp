...
CStarField::CStarField(unsigned int nNumStars, float fGamma, float fBrightness, 
					   float fSpeed, float fZoom, float fExpanse, bool bMoveCamera)
		  : m_pStars(0)
...
int CStarField::RenderFrame(void)
{
	...
	m_fVelocity += (m_fMaxVelocity - m_fVelocity) * .0001f;
		
	if (m_bMoveCamera) {
		m_Cam.aacc	= (m_Cam.aacc + RangeRand(-.00001f, .00002f)) * (.99f/1.05f);
		m_Cam.bacc	= (m_Cam.bacc + RangeRand(-.00001f, .00002f)) * (.99f/1.05f);
		m_Cam.cacc	= (m_Cam.cacc + RangeRand(-.00001f, .00002f)) * (.992f/1.05f);
	...
	for (unsigned int n = 0; n < m_nStarCnt; n++)
	{
		...
		int index = ((dist >> 1) < 0) ? 0 : dist >> 1;

		float c = m_fBrightTable[min(index, 255)];
...
void CStarField::DoDraw()
{
	...
	if (FAILED(m_pd3dDevice->CreateVertexBuffer(nVSize, NULL, D3DFVF_CUSTOMVERTEX, D3DPOOL_DEFAULT, &pVB, NULL)))
		return;	

	VOID* pVertices;

	pVB->Lock(0, nVSize, (void**)&pVertices, 0);
    ...
    m_pd3dDevice->SetStreamSource(0, pVB, 0, sizeof(ST_CUSTOMVERTEX));
