...
public:
	...
	void Create(LPDIRECT3DDEVICE9 device, int width, int height);	
	bool Start();
	void Stop();
	void Render();
...
private:
	...
	int					m_height;
	int					m_width;
	IDirect3DTexture9*	m_bufferTexture;	// Workspace area where worms are drawn
	IDirect3DTexture9*	m_colMapTexture;	// Used for remapping the colours
	IDirect3DTexture9*	m_spriteTexture;	// Texture for the worm sprite
	LPDIRECT3DPIXELSHADER9 m_pShader;			// Pixel shader
	LPD3DXBUFFER		m_pUcode;
